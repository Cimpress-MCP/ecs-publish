...
function paginateRules (region, params, rules = []) {
  const elb = new ELBv2({ region: region });
  return elb.describeRules(params).promise()
    .then((data) => {
      rules = rules.concat(data.Rules);
      if (data.NextMarker) {
        params.Marker = data.NextMarker;
        return paginateRules(region, params, rules);
      }
      return rules;
    })
    .catch((err) => {
      console.error(err);
      throw new Error('Unable to describe rules');
    });
}
  ...
  return paginateListeners(cfg.region, { LoadBalancerArn: cfg.loadBalancerArn })
    .then((listeners) => {
      ...
      return Promise.all(cfg.listeners.map((listener) => {
        return paginateRules(cfg.region, { ListenerArn: listener.ListenerArn });
      }));
    })
    .then((rulePromises) => {
...
module.exports.ensureCertificateIsInLoadBalancer = function ({cfg}) {
  if (!cfg.targetGroup || !cfg.service || !cfg.service.loadBalancer || !cfg.service.hostedZoneId) return {cfg};
